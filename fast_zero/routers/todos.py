...
@router.get('/', status_code=HTTPStatus.OK, response_model=ToDoListSchema)
async def resume_todos(
    session: Session,
...
@router.patch('/{todo_id}', response_model=ToDoUpdateSchema)
async def pacth_todo(
    todo_id: int,
    session: Session,
    current_user: CurrentUser,
